...
import cn.knet.domain.entity.KnetPermission;
import cn.knet.domain.entity.KnetUser;
import cn.knet.domain.enums.StatusEnum;
import cn.knet.domain.mapper.KnetPermissionMapper;
...
public class MyRealm extends AuthorizingRealm {
    ...
    protected AuthorizationInfo doGetAuthorizationInfo(PrincipalCollection principals) {
        ...
        if (user != null) {
            try {
                List<KnetPermission> l = knetPermissionMapper.selectList(new QueryWrapper<KnetPermission>().eq("ID", user.getId()));
                if (l != null && l.size() > 0) {
